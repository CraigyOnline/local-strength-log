  ...
  return (
    <div className="flex flex-col gap-6 px-4 pt-6">
      ...
      <header className="grid grid-cols-[auto_minmax(0,1fr)] items-center gap-4">
        <div className="grid h-16 w-16 shrink-0 place-items-center rounded-full text-2xl font-bold bg-primary text-primary-foreground">
          🏋️
        </div>
      ...
      <section className="rounded-2xl bg-card p-5 border border-border/50 shadow-sm">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          {(() => {
            const topMuscle = Object.entries(intensity)
              .sort((a, b) => b[1] - a[1])[0]?.[0];

            return topMuscle ? (
              <div className="text-right">
                <p className="text-[10px] text-muted-foreground">Top muscle</p>
        ...
        {/* MAP */}
        <div className="rounded-xl bg-secondary/20 p-3 mb-5">
            ...
            {(workouts ?? []).map((w) => (
              <li key={w.id}>
                <Link
                  ...
                  className="block rounded-2xl bg-card p-4 hover:bg-secondary/40 transition-colors"
                >
                  <p className="font-semibold truncate">{w.name}</p>
                  <p className="text-xs text-muted-foreground">
                    {new Date(w.startedAt).toLocaleDateString()} · {formatDuration(w.durationSec)}
                  </p>
                </Link>
              </li>
            ))}
